...
def parallel_stable_pc(data, test, alpha=0.05, processes=32, max_level=None):
    cols = data.columns
    ...
    for lvl in lvls:
        configs = [(i, j, lvl) for i, j in product(cols_map, cols_map) if i != j and graph[i][j] == 1 and (graph[i]==1).sum() > lvl + 1]
        if not configs:
            break
        ...
        with Pool(processes=processes, initializer=_init_worker,
                  initargs=(data_raw, data.shape, graph_raw, vertices, test, alpha)) as pool:
